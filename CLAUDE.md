...
Synara is a minimal web GUI for using coding agents. It is multi-provider: `ProviderKind` currently spans 10 providers — Codex, Claude (`claudeAgent`), Cursor, Devin, Antigravity, Grok, Factory Droid (`droid`), OpenCode, Pi, and Prime. Each provider has its own model options and capabilities (reasoning effort, thinking budget/level, context window, fast mode), defined in `packages/contracts` and resolved in `packages/shared/src/model.ts`.
