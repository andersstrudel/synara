...
import { PrimeAdapter, PrimeAdapterShape } from "../Services/PrimeAdapter.ts";
...
const fakePrimeAdapter: PrimeAdapterShape = {
  provider: "prime",
  capabilities: { sessionModelSwitch: "restart-session" },
  startSession: vi.fn(),
  sendTurn: vi.fn(),
  ...
  Layer.mergeAll(
    Layer.provide(
      ...
      Layer.mergeAll(
        ...
        Layer.succeed(PrimeAdapter, fakePrimeAdapter),
      ),
    ),
    NodeServices.layer,
  ),
...
layer("ProviderAdapterRegistryLive", (it) => {
  it.effect("resolves a registered provider adapter", () =>
    Effect.gen(function* () {
      ...
      const prime = yield* registry.getByProvider("prime");
      ...
      assert.equal(prime, fakePrimeAdapter);
