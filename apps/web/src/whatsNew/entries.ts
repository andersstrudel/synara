    ...
    features: [
      {
        ...
        details:
          "Synara starts `prime-agent --mode acp` for each thread, resumes threads through Prime's own session ids, lists every model its credentials unlock grouped by upstream provider with a per-model thinking-level picker, runs `/compact` natively, exposes Prime skills and slash commands, and passes its browser and gateway MCP tools into the session. The model picker's Effort header adds the same lightning Fast toggle Codex shows for the GPT-5.4, GPT-5.5, and GPT-5.6 models Prime reaches through ChatGPT Codex, applied through Prime's own service tier. Authentication stays owned by Prime through `/login`, whose credentials Synara reads from `~/.prime/agent/auth.json` without duplicating them. Plan mode is a prompt-prefix emulation, live-turn steering is not advertised, and the approval-required runtime mode is unavailable because Prime never requests permissions.",
      },
    ],
  },
  {
